//
//  SplineReducer6+ReduceDouble.swift
//  Jiggle3
//
...
extension SplineReducer6 {
    ...
    func populateTempZipperPouches_ReduceDouble(maxCombinedPouches: Int) {
        ...
        for zipperPouchIndex in 0..<zipperPouchCount {
            ...
            if zp_cur.isVisited == false {
                ...
                if indexBck2 == -1 { indexBck2 = zipperPouchCount - 1 }
                
                // Will combining (back 1) and (back 2) overflow?
                let zp_b_2 = zipperPouches[indexBck2]
                let zp_b_1 = zipperPouches[indexBck1]
                let combinedCountBck = zp_b_2.numberOfCombinedZipperPouches + zp_b_1.numberOfCombinedZipperPouches
    ...
    func executeCommand_ReduceDouble(numberOfPointsSampledForEachControlPoint: Int,
        ...
        for _ in 0..<tryCount {
            ...
            // Did we underflow?
            if zipperPouchCount <= Self.minZipperPouchCount { break }
            
            // Load up the temporary zipper pouches.
            populateTempZipperPouches_ReduceDouble(maxCombinedPouches: maxCombinedPouches)
            
            // Did we find any temporary zipper pouches?
            if tempZipperPouchCount <= 0 { break }
            ...
            while (loopIndex < tempZipperPouchCount) && (isLoopingThroughoutZipperPouches == true) {
                ...
                if attemptToReduceDouble(index: zipperPouchIndex,
        ...
        print("ReduceDouble => tolerance = \(tolerance) (squared = \(toleranceSquared))")
        print("ReduceDouble => KP_tryCount = \(KP_tryCount) / \(tryCount)")
    ...
    //  Start: [a]...[b]...[zipperPouchIndex]...[c]...[d]
    // Finish: [a].........[zipperPouchIndex].........[d]
    // So, we need to check:
    //    [s1] [a].........[zipperPouchIndex].........[d][s2]
    //    [    sample a   ][             sample b           ]
    func attemptToReduceDouble(index: Int,
        ...
        if indexBck3 == -1 { indexBck3 = zipperPouchCount - 1 }
        ...
        for zipperPouchIndex in 0..<zipperPouchCount {
            if (zipperPouchIndex != indexBck1) && (zipperPouchIndex != indexFwd1) {
                let zipperPouch = zipperPouches[zipperPouchIndex]
        ...
        // ...[b3][b2][b1][self][f1][f2][f3]...
        let zp_b_3 = zipperPouches[indexBck3]
        let zp_b_2 = zipperPouches[indexBck2]
        let zp_b_1 = zipperPouches[indexBck1]
        ...
        // two zipper pouches...
        let newCount = zipperPouchCount - 2
        let newIndex: Int
        if indexBck1 > index {
        ...
        let count_b_3 = zp_b_3.numberOfCombinedZipperPouches * numberOfPointsSampledForEachControlPoint
        ...
        let numberOfCombinedZipperPouchesBack2 = zp_b_2.numberOfCombinedZipperPouches + zp_b_1.numberOfCombinedZipperPouches
        let count_b_2 = numberOfCombinedZipperPouchesBack2 * numberOfPointsSampledForEachControlPoint
        ...
        SplineReducer6ZipperPouch.transferAllSegments(from: zp_b_1, to: zp_b_2)
        
        // We "combine" b1 and b2...
        zp_b_2.numberOfCombinedZipperPouches += zp_b_1.numberOfCombinedZipperPouches
        ...
        // We remove b1 and f1...
        removeZipperPouchTwo(index1: indexBck1, index2: indexFwd1)
