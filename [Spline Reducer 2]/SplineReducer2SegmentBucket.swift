//
//  SplineReducer2SegmentBucket.swift
//  Jiggle3
//
...
final class SplineReducer2SegmentBucket {
    
    private class SplineReducer2SegmentBucketNode {
        var splineReducerSegments = [SplineReducer2Segment]()
        var splineReducerSegmentCount = 0
        
        func remove(_ splineReducerSegment: SplineReducer2Segment) {
        ...
        func add(_ splineReducerSegment: SplineReducer2Segment) {
    ...
    private var grid = [[SplineReducer2SegmentBucketNode]]()
    private var gridX: [Float]
    private var gridY: [Float]
    
    private(set) var splineReducerSegments: [SplineReducer2Segment]
    ...
    init() {
        ...
        splineReducerSegments = [SplineReducer2Segment]()
        
        var x = 0
        while x < Self.countH {
            var column = [SplineReducer2SegmentBucketNode]()
            var y = 0
            while y < Self.countV {
                let node = SplineReducer2SegmentBucketNode()
                column.append(node)
                y += 1
            }
            grid.append(column)
    ...
    func build(splineReducerSegments: [SplineReducer2Segment], splineReducerSegmentCount: Int) {
        ...
        let referenceSplineReducer2Segment = splineReducerSegments[0]
        
        var minX = min(referenceSplineReducer2Segment.x1, referenceSplineReducer2Segment.x2)
        var maxX = max(referenceSplineReducer2Segment.x1, referenceSplineReducer2Segment.x2)
        var minY = min(referenceSplineReducer2Segment.y1, referenceSplineReducer2Segment.y2)
        var maxY = max(referenceSplineReducer2Segment.y1, referenceSplineReducer2Segment.y2)
    ...
    func remove(splineReducerSegment: SplineReducer2Segment) {
        let _minX = min(splineReducerSegment.x1, splineReducerSegment.x2)
        let _maxX = max(splineReducerSegment.x1, splineReducerSegment.x2)
        let _minY = min(splineReducerSegment.y1, splineReducerSegment.y2)
        let _maxY = max(splineReducerSegment.y1, splineReducerSegment.y2)
    ...
    func add(splineReducerSegment: SplineReducer2Segment) {
    ...
    func query(splineReducerSegment: SplineReducer2Segment) {
    ...
    func query(splineReducerSegment: SplineReducer2Segment, padding: Float) {
        let x1 = splineReducerSegment.x1
        let y1 = splineReducerSegment.y1
        let x2 = splineReducerSegment.x2
        let y2 = splineReducerSegment.y2
